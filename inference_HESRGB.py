...
def get_flags():
    ...
    parser.add_argument('--yaml_config', type=str, default='/ghome/xiaozy/EventVideoFrameInterpolation/evfi_inference_weng/config_weng/test_hsergb_EVFINet_ours.yml')
...
#         param_group['lr'] = lr
    ...
    for i, (inputs) in enumerate(tqdm(dataloader, total=len(dataloader))):
        ...
        if h>1000 and w>1000:
            frame0 = frame0[:,:,0:900,0:900]
            frame1 = frame1[:,:,0:900,0:900]
        else:
        ...
        for idx in range(len(inputs['event0'])):
            if h>1000 and w>1000:
                left_event = inputs['event0'][idx][:,:,0:900,0:900].to(device)
                right_event = inputs['event1'][idx][:,:,0:900,0:900].to(device)
                left_event_cnt = inputs['cnt0'][idx][:,:,0:900,0:900].to(device) # only for visualization
                right_event_cnt = inputs['cnt1'][idx][:,:,0:900,0:900].to(device) # only for visualization
                gt = inputs['gt'][idx][:,:,0:900,0:900].to(device)
            else:
            ...
            # gt = tensor2np(gt.detach()[0])
            # pred_frame = tensor2np(pred_frame.detach()[0])
            cv2.imwrite(os.path.join(step_dir, 'frame0.png'), fframe0[:, :, [2, 1, 0]])
            cv2.imwrite(os.path.join(step_dir, 'frame1.png'), fframe1[:, :, [2, 1, 0]])
            ...
            img1 = cv2.imread(os.path.join(step_dir, 'gt.png'))
            img2 = cv2.imread(os.path.join(step_dir, 'prediction.png'))
            res = np.abs(np.float32(img1)-np.float32(img2))
            cv2.imwrite(os.path.join(step_dir, 'error.png'),res)

            plt.figure()
            img = io.imread(os.path.join(step_dir, 'error.png'))
            img = color.rgb2gray(img)*255
            plt.imshow(img, cmap=plt.cm.hot, vmin=0, vmax=80)
            plt.colorbar()
            plt.imsave(os.path.join(step_dir, 'myerror.png'),img,cmap=plt.cm.hot, vmin=0, vmax=80)
